...
diagonal_flag = True # Set this flag = 0 for non-diagonal sudoku
unitlist_1 = row_units + column_units + square_units
...
def naked_twins(values):
    ...
    for unit in unitlist_1:  # for without diagonal
        ...
        inv_twin_dict = {key: value for key, value in inverse_map.items() if len(value) == 2 and len(key) == 2} #possible twin
...
def eliminate(values):
    """Eliminate values using the constraints.
        Args:
            values(dict): a dictionary of the form {'box_name': '123456789', ...}

        Returns:
            the values dictionary with the same value eliminated from peers.
        """
...
def reduce_puzzle(values):
    """Eliminate values using constraints propagation strategy.
            Args:
                values(dict): a dictionary of the form {'box_name': '123456789', ...}
